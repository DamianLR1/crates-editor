...
import { Box, Home, Gift, Settings2, LayoutGrid, Clapperboard, Dices, ArrowUpCircle, FolderOpen, FileUp, FilePlus2, Search } from 'lucide-react';
...
const NAV = [
  ...
  { id: 'openings', label: 'Openings', icon: Clapperboard },
...
export default function Sidebar({ section, setSection }) {
  const { model, fileName, server, switchCrate, openServer, openFile, newBlankFile, goHome } = useCrate();
  ...
  return (
    <aside className="w-64 shrink-0 h-screen sticky top-0 flex flex-col border-r border-ink-700 bg-ink-950/70">
      <button onClick={goHome} title="Volver al inicio" className="flex items-center gap-2.5 px-5 py-4 border-b border-ink-700 text-left hover:bg-ink-900 transition-colors">
      ...
      <nav className="p-3 space-y-1">
        {NAV.map(({ id, label, icon: Icon }) => (
          <button
            key={id}
            onClick={() => (id === 'home' ? goHome() : setSection(id))}
